    ...
    <section id="portfolio" className="py-10 bg-gray-50">
      <div className="container mx-auto px-6">
        ...
        <Tabs defaultValue="ux-ui" className="w-full max-w-4xl mx-auto">
          <TabsList className="grid w-full grid-cols-3 bg-white border mb-4 rounded-full">
            <TabsTrigger 
              value="ux-ui" 
              className="text-gray-700 data-[state=active]:bg-[#ffcc00] data-[state=active]:text-gray-900 rounded-full text-xs"
            >
              UX/UI Design
            </TabsTrigger>
            <TabsTrigger 
            ...
            <TabsContent key={category} value={category}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {items.map((item, index) => (
                  <div 
                    key={index}
                    className="group relative overflow-hidden rounded-2xl bg-white border hover:transform hover:scale-105 transition-all duration-300 hover:shadow-lg"
                  >
